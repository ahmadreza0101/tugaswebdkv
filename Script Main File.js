...
if (carousel) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    let modeToggles = document.querySelectorAll(".modeToggle"); // Ambil semua tombol mode dengan class

    // Jika tombol tidak ditemukan, tambahkan tombol secara otomatis
    if (modeToggles.length === 0) {
        console.warn("⚠️ Tidak ada tombol `.modeToggle` ditemukan! Menambahkan tombol otomatis...");
        const newButton = document.createElement("button");
        newButton.classList.add("modeToggle");
        newButton.textContent = "🌙";
        document.body.appendChild(newButton);
        modeToggles = document.querySelectorAll(".modeToggle"); // Perbarui daftar tombol
    }

    // Cek apakah user sudah pernah memilih mode, jika belum gunakan mode perangkat
    if (localStorage.getItem("nightMode") === null) {
        const prefersDarkMode = window.matchMedia("(prefers-color-scheme: dark)").matches;
        if (prefersDarkMode) {
            applyNightMode(); // Terapkan mode gelap jika perangkat menggunakan dark mode
        } else {
            disableNightMode(); // Terapkan mode terang jika perangkat menggunakan light mode
        }
    }

    // Pastikan mode gelap tetap aktif jika sebelumnya sudah dipilih
    if (localStorage.getItem("nightMode") === "enabled") {
        setTimeout(applyNightMode, 50); // Beri sedikit delay untuk memastikan elemen termuat dulu
    }

    // Tambahkan event listener ke semua tombol modeToggle
    ...
    function applyNightMode() {
        ...
        document.querySelectorAll(".section, .hero, .card, .cards, .cardp, .contact-card, h1, h2, h3, h4, h5, h6, p, span, footer, button, a")
            .forEach(el => el.classList.add("night-mode"));
    ...
    function disableNightMode() {
        ...
        document.querySelectorAll(".section, .hero, .card, .cards, .cardp, .contact-card, h1, h2, h3, h4, h5, h6, p, span, footer, button, a")
            .forEach(el => el.classList.remove("night-mode"));
...
const news = [
  ...
  },
  {
      ...
      content: "Dalam dunia pendidikan, pendekatan pembelajaran yang relevan dengan kebutuhan dunia kerja sangat penting, terutama bagi murid Sekolah Menengah Kejuruan (SMK). Dua pendekatan yang sering digunakan adalah...."
  },
  {
      title: "Arah Kebijakan Pendidikan 2025",
      link: "https://www.dkv.smkn1.tangera.ng/news/2025-01/rapikan-tempat-tidurmu",
  ...
  },
  {
      ...
      content: "Alhamdulillah, di awal tahun 2025 ini DKV telah mempunyai website (yang sedang anda lihat ini) dan Learning Management Sistem (LMS) dengan alamat web adalah lms.dkv.smkn1.tangera.ng. LMS ini dibangun dengan memakai CMS Moodle yang telah dipakai oleh..."
  },
];
